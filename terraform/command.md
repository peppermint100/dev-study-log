...
terraform fmt 명령어 옵션
* terraform fmt -recursive
  * 현재 디렉터리뿐만 아니라 **모든 하위 디렉터리**에 있는 .tf 파일을 재귀적으로 찾아 서식을 일괄 정리한다.
  * 프로젝트 전체의 코드 스타일을 한 번에 통일할 때 가장 유용하다. 문제에서 요구한 상황(현재 디렉터리와 modules 하위 디렉터리 모두 포맷팅)에 가장 적합한 명령어이다.
* terraform fmt -check
  * 파일 내용을 **수정하지 않고**, 서식에 맞지 않는 파일이 있는지 **검사**만 한다.
  * 서식이 맞지 않는 파일이 있으면 목록을 출력하고, 0이 아닌 종료 코드를 반환한다.
  * CI/CD 파이프라인에서 코드 스타일이 올바른지 검증하는 용도로 주로 사용된다.
* terraform fmt -diff
  * 파일 내용을 **수정하지 않고**, 서식을 적용했을 때 어떤 부분이 **변경되는지 diff 형식으로 보여준다**.
  * 실제 변경 전에 어떤 부분이 수정될지 미리 확인하고 싶을 때 사용한다.
* terraform fmt -list=false
  * fmt 명령어는 기본적으로 서식을 변경한 파일의 목록을 출력한다 (-list=true가 기본값).
  * 이 옵션을 사용하면 서식을 변경하더라도 파일 목록을 **출력하지 않는다**.
  * 스크립트 등에서 불필요한 출력을 숨기고 싶을 때 사용한다. 이 옵션 자체는 하위 디렉터리 처리와 관련이 없다.

## terraform graph 
Terraform 구성에 정의된 리소스들의 **의존 관계와 연결 구조를 시각적으로 표현**하기 위한 데이터를 생성한다.

이 명령어는 직접 이미지를 만들어주지는 않고, **DOT이라는 그래프 서술 언어** 형식으로 결과를 출력한다. 이 결과를 시각화 도구에 입력하면 의존성 그래프 이미지를 얻을 수 있다.

## terraform init -upgrade 
옵션은 Terraform 초기화 과정에서 **프로바이더(provider)와 모듈(module)을 최신 허용 버전으로 업데이트**하는 데 사용된다.

terraform init은 일반적으로 잠금 파일(.terraform.lock.hcl)에 명시된 버전을 설치하지만, -upgrade 옵션을 추가하면 이 잠금 파일을 무시하고 구성 파일(.tf)의 버전 제약 조건 내에서 가장 최신 버전을 찾아 설치한다.
